...
# read data -----------------------------------------------------------------------------------------
lam     <- read.csv( paste0(args[2], 'lambdas_6tr.csv'), stringsAsFactors = F) 
m_info  <- read.csv( paste0(args[2], 'MatrixEndMonth_information.csv'), stringsAsFactors = F)
clim    <- read.csv( paste0(args[2], 'pet_fc_demo.csv'), stringsAsFactors = F) #%>%
#mutate( ppt = ppt / 30)
spp     <- clim$species %>% unique
spp     <- spp[-c(13,18,22,27,28,29,30,32,33)]
